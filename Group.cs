...
namespace grafiti
{
...

    {
        ...
        private List<GlobalGestureRecognizer> m_ggrs;
        ...
        internal List<GlobalGestureRecognizer> GGRs { get { return m_ggrs; } }
        ...
        public Group(bool intersectionMode)
		{
            ...
            m_ggrs = new List<GlobalGestureRecognizer>();
        ...
        public void AddGGR(GlobalGestureRecognizer ggr)
...

        {
...

            {
                ...
                foreach (GestureRecognizer gr in target.GetLocalGRs())

                    gr.Process(trace);

            }

            foreach (GlobalGestureRecognizer mtgr in m_ggrs)
        ...
        public void UpdateTargets(Trace trace, int state)

        {

            List<IGestureListener> traceInitialTargets;

            List<IGestureListener> traceEnteringTargets;

            List<IGestureListener> traceLeavingTargets;

            List<IGestureListener> traceFinalTargets;
            ...
            if (state == (int)TuioCursor.States.add)

            {

                traceInitialTargets = trace.InitialTargets;
            ...
            traceEnteringTargets = trace.EnteringTargets;

            traceLeavingTargets = trace.LeavingTargets;



            if (m_newInitialTargets.Count > 0 && state != (int)TuioCursor.States.add)
            ...
            if (m_leavingTargets.Count > 0 && state != (int)TuioCursor.States.add)
...

            {
...

                {
                    ...
                    m_intersectionTargets.Remove(traceLeavingTarget);
            ...
            if (state == (int)TuioCursor.States.del)

            {

                traceFinalTargets = trace.FinalTargets;
            ...
            if (newCurrent | newIntersect | newUnion | newInitial | newFinal | newEntering | newLeaving)
                ...
                foreach (GlobalGestureRecognizer mtgr in m_ggrs)
                    ...
                    mtgr.UpdateHandlers(newInitial, newFinal, newEntering, newCurrent, newLeaving, newIntersect, newUnion);
